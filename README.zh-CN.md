...
一个 **AI 驱动的加密货币合约交易**平台，可在真实市场中运行多个基于 LLM 的交易智能体，提供完整的监控仪表板、排行榜和 AI 助手服务，由 **ROMA（递归开放元智能体）**框架驱动。
...
本平台提供面向实盘交易运营的 **前端交易控制台**，具备以下能力：
- 🏆 **排行榜**：实时并排比较多个 AI 交易智能体的表现
- 📊 **性能可视化**：跟踪所有智能体的账户价值、盈亏和交易指标
- 🎨 **智能体总览**：查看每个将任意 DEX 账户与任意 LLM 模型组合的交易智能体
- 📈 **实时交易面板**：监控持仓、已完成交易和 AI 决策过程
- 📝 **自定义提示词**：为每个智能体配置用户定义的交易策略
- 💬 **AI 聊天助手与币种分析**：通过交互式聊天获取交易建议、提示词建议、代币分析和平台使用指导

界面重点突出 **透明度和可观测性**，帮助你清晰理解每个智能体在做什么、为什么这么做，以及对整体账户风险和收益的影响。
...
- **币种分析**：请求针对单个代币的分析（例如 “分析 BTC”、“ETH 现在应该怎么做？”）
- **平台帮助**：获取理解平台功能和特性的帮助
- **实时响应**：由用于交易决策的相同 LLM 模型提供支持
- **双语支持**：完整支持英文和中文的国际化

整体界面强调 **清晰、透明与可控性**，既关注结果表现，也帮助你理解 AI 驱动交易背后的推理逻辑。
